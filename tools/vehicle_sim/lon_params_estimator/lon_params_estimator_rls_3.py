...
class LonParamsEstimatorRLS3(LonParamsEstimator):
    def __init__(self, estimator_params, vehicle_params):
        super(LonParamsEstimatorRLS3, self).__init__(estimator_params, vehicle_params)
    ...
    def update(self, controller_input):
        ...
        phi = np.array([[self._v**2, 
                         self._acc - g * math.sin(self._pitch) + g * math.cos(self._pitch) * self._nominal_rolling_coefficient]])
        ...
        self._vehicle_mass_estimated = max(min(state[1,0],self._vehicle_mass_high_bound),self._vehicle_mass_low_bound)
