...
class RGEF2TLSEstimator(BaseRLSEstimator):
    def __init__(self, init_state, init_P, config):
        super(RGEF2TLSEstimator, self).__init__(init_state, init_P, config)
    ...
    def update(self, y):
        ...
        delta = 0.01
        F = self._forget_factor * np.identity(self._state_num+1) + delta * self._P
