...
class LonParamsEstimatorRLS4(LonParamsEstimator):
    def __init__(self, estimator_params, vehicle_params):
        super(LonParamsEstimatorRLS4, self).__init__(estimator_params, vehicle_params)
        ...
        init_state = np.array([[self._nominal_vehicle_mass]])
    ...
    def update(self, controller_input):
        ...
        self._noise_covariance = self._noise_covariance_estimator.update([phi[0,0], y])
        ...
        self._vehicle_mass_estimated = max(min(state[0,0],self._vehicle_mass_high_bound),self._vehicle_mass_low_bound)
